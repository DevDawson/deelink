...
function PaymentCompleteInner() {
  ...
  const poll = async () => {
    ...
    try {
      ...
      // Network error — keep trying up to max
      pollCount.current += 1
      if (pollCount.current >= MAX_POLLS) {
        setStatus('processing')
        return
  ...
  return (
    <div className="min-h-screen bg-[#050508] flex items-center justify-center px-4">
      <div className="absolute -top-40 left-1/2 -translate-x-1/2 w-[500px] h-[500px] bg-violet-700/10 rounded-full blur-[100px] pointer-events-none" />
      ...
      <div className="relative glass rounded-2xl p-10 max-w-sm w-full text-center">
        ...
        {status === 'loading' && (
          <>
            ...
            <h2 className="text-white font-bold text-lg">Confirming payment…</h2>
            <p className="text-slate-500 text-sm mt-1">Verifying with Pesapal, please wait.</p>
          </>
        )}
        ...
        {status === 'success' && (
          <>
            ...
            {plan && (
              <p className="text-violet-400 text-sm font-semibold mt-1 capitalize">{plan} plan activated</p>
            )}
            <p className="text-slate-400 text-sm mt-2 mb-6">Redirecting to dashboard…</p>
            <Link
              href="/dashboard"
        ...
        {status === 'processing' && (
          <>
            ...
            <h2 className="text-white font-bold text-xl">Payment processing</h2>
        ...
        {status === 'failed' && (
          <>
            ...
            <h2 className="text-white font-bold text-xl">Payment failed</h2>
            <p className="text-slate-400 text-sm mt-2 mb-6">
        ...
        {status === 'no_tracking' && (
          <>
            ...
            <h2 className="text-white font-bold text-xl">No payment found</h2>
            <p className="text-slate-400 text-sm mt-2 mb-6">
