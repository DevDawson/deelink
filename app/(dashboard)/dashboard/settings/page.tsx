...
const inputClass =
  'w-full rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-transparent transition-colors text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-600'
...
export default function SettingsPage() {
  ...
  if (!artist || !form) {
    return (
      <div className="flex justify-center py-12">
        <Loader className="animate-spin text-[var(--muted)]" size={24} />
      </div>
    )
  }
  ...
  return (
    <div className="space-y-5 pb-12">
      ...
      <div className="glass rounded-2xl overflow-hidden">
        ...
        >
          ...
          <input ref={coverRef} type="file" accept="image/*" className="hidden"
            onChange={(e) => { const f = e.target.files?.[0]; if (f) setCoverSrc(URL.createObjectURL(f)) }} />
        </div>
        ...
        <div className="px-5 pb-5">
          ...
          <div className="relative -mt-9 mb-4 w-fit">
            <div
              className="w-[72px] h-[72px] rounded-full border-4 border-[var(--background)] overflow-hidden bg-slate-200 dark:bg-slate-800 cursor-pointer group relative"
            ...
            >
              {avatarSrc ? (
                ...
                <div className="w-full h-full flex items-center justify-center text-2xl font-black text-slate-500 dark:text-slate-400">
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-xs font-medium text-[var(--muted)] uppercase tracking-wide mb-1.5">
                Display name
              </label>
              <input type="text"
            ...
            <div>
              <label className="block text-xs font-medium text-[var(--muted)] uppercase tracking-wide mb-1.5">Bio</label>
              <textarea
              ...
              <div className="relative">
                ...
                {key === 'password' && (
                  <button type="button" onClick={() => setShowPw(!showPw)}
                    className="absolute right-3 top-1/2 -translate-y-1/2 text-[var(--muted)] hover:text-slate-900 dark:hover:text-white transition-colors">
      ...
      {/* Danger zone */}
      <div className="border border-red-400/30 dark:border-red-500/20 rounded-2xl p-5 bg-red-50 dark:bg-red-500/5">
        ...
        <div className="space-y-3">
          <div>
            <label className="block text-xs text-[var(--muted)] mb-1.5">
              Type <span className="font-mono text-red-500 font-bold">{artist.username}</span> to confirm
            </label>
            <input type="text" value={deleteConfirm}
              onChange={(e) => setDeleteConfirm(e.target.value)}
              className="w-full rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-red-500/50 text-slate-900 dark:text-white"
              style={{ background: 'var(--input-bg)', border: '1px solid rgba(239,68,68,0.25)' }}
              placeholder={artist.username} />
          </div>
          <div>
            <label className="block text-xs text-[var(--muted)] mb-1.5">Your password</label>
            <input type="password" value={deletePw}
              onChange={(e) => setDeletePw(e.target.value)}
              className="w-full rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-red-500/50 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-600"
              style={{ background: 'var(--input-bg)', border: '1px solid rgba(239,68,68,0.25)' }}
              placeholder="••••••••" />
          </div>
